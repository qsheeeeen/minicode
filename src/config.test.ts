...
describe('loadConfigSync', () => {
  ...
  it('parses valid JSON config', () => {
    vi.mocked(fs.readFileSync).mockReturnValue('{"model": "glm-4.7@zhipu", "promptFile": "CUSTOM.md"}');
...
describe('parseModelSpecifier', () => {
  it('parses model@provider format', () => {
    const result = parseModelSpecifier('claude-3@anthropic', { anthropic: { apiKey: 'key' } });
    expect(result?.modelName).toBe('claude-3');
    expect(result?.providerName).toBe('anthropic');
  });

  it('falls back to first provider when no @provider', () => {
    const result = parseModelSpecifier('claude-3', { anthropic: { apiKey: 'key' } });
    expect(result?.providerName).toBe('anthropic');
  });
