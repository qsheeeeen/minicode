...
export function ModelSelectInput({
  ...
  providers = {},
  tiers = {},
}: InputComponentProps & { providers?: Record<string, ProviderConfig>; tiers?: Record<string, string> }) {
  const [step, setStep] = useState<'main' | 'edit-tier' | 'provider' | 'model'>('main');
  ...
  if (step === 'main') {
    ...
    const options = [
      ...tierLabels.map(t => ({ label: `Tier ${t} → ${tiers[t] || '(unset)'}`, value: `${t}:` })),
      { label: 'Edit tier mapping...', value: '_edit_' },
    ];

    return (
      <Box flexDirection="column">
        <Text bold color="magenta">Select tier:</Text>
  ...
  if (step === 'edit-tier') {
    ...
    return (
      <Box flexDirection="column">
        <Text bold color="magenta">Edit which tier?</Text>
        <Select options={options} onChange={(v) => { setEditTier(v); setStep('provider'); }} />
  ...
  if (step === 'provider') {
    ...
    return (
      <Box flexDirection="column">
        <Text bold color="magenta">Provider for Tier {editTier}:</Text>
        {options.length > 0 ? (
  ...
  const options = Object.keys(providers[selectedProvider]?.models ?? {})
    .map(k => ({ label: k, value: `${editTier}:${k}@${selectedProvider}` }));

  return (
    <Box flexDirection="column">
      <Text bold color="magenta">Model for Tier {editTier} @{selectedProvider}:</Text>
      {options.length > 0 ? (
        <Select options={options} onChange={(v) => onExecute?.(v)} />
      ) : (
