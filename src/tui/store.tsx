...
export type TuiAction =
  ...
  | { type: 'SET_INPUT_MODE'; payload: { mode: string; props?: Record<string, unknown> } }
  | { type: 'SET_INPUT_VALUE'; payload: string }
  | { type: 'INCREMENT_INPUT_KEY' }
  | { type: 'SET_SESSION_LIST'; payload: { sessions: Array<{ name: string }>; selectedIndex?: number } }
  ...
  | { type: 'SET_PENDING_PROMPT'; payload: (Prompt & { resolve: (value: string) => void }) | null };
    ...
    case 'SET_SESSION_LIST':
      return {
        ...
        sessionList: {
          ...
          selectedIndex: action.payload.selectedIndex ?? state.sessionList.selectedIndex,
        },
      };
...
const TuiDispatchContext = createContext<React.Dispatch<TuiAction> | undefined>(undefined);

export function TuiProvider({ children, initialState: init = initialState }: { children: ReactNode; initialState?: Partial<TuiState> }) {
  const [state, dispatch] = useReducer(tuiReducer, { ...initialState, ...init });
