...
export function InputArea({ agentRef, handleSubmit }: InputAreaProps) {
  ...
  useInput((_input, key) => {
    if (input.mode !== 'chat' || isModal || matchingCommands.length === 0) return;
    if (key.upArrow) {
      ...
      dispatch({ type: 'SET_INPUT_VALUE', payload: `/${matchingCommands[selectedSuggestion].name} ` });
      dispatch({ type: 'INCREMENT_INPUT_KEY' });
      setSelectedSuggestion(0);
    }
  }, { isActive: input.mode === 'chat' && !isModal && matchingCommands.length > 0 });
  ...
  const handleSubmitValue = useCallback(async (value: string) => {
    if (input.mode === 'effort-select') {
      ...
      dispatch({ type: 'ADD_MESSAGE', payload: { role: 'status', content: `(Effort set to: ${value})`, timestamp: new Date() } });
      ...
      if (tierMatch) {
        ...
        if (modelSpec) {
          ...
          if (parsed) {
            ...
            dispatch({ type: 'ADD_MESSAGE', payload: { role: 'status', content: `(Model set to: ${modelSpec})`, timestamp: new Date() } });
          }
        }
      }
      dispatch({ type: 'SET_INPUT_MODE', payload: { mode: 'chat' } });
