...
describe('InputArea Component', () => {
  const mockLoadingRef = (loading: boolean): React.MutableRefObject<boolean> => {
  ...
  it('renders nothing when there is a pending prompt (isModal = true)', () => {
    ...
    const { lastFrame } = render(
      <TuiProvider initialState={{
        ...
        <InputArea agentRef={mockAgentRef} handleSubmit={vi.fn()} loadingRef={mockLoadingRef(false)} />
      </TuiProvider>
    );
  ...
  it('renders chat input by default', () => {
    const mockAgentRef = { current: { getStore: () => ({ addStatus: vi.fn() }) } } as any;
  ...
  it('shows … prompt when isLoading is true', () => {
    ...
    const { lastFrame } = render(
      <TuiProvider initialState={{
        ...
        <InputArea agentRef={mockAgentRef} handleSubmit={vi.fn()} loadingRef={mockLoadingRef(true)} />
      </TuiProvider>
    );
