...
export { SkillRegistry, type SkillMeta } from './skill-registry.js';
...
const initSkill: SkillMeta = {
  ...
  body: `Set up a minimal ${promptFile} (and optionally skills and hooks) for this repo. ${promptFile} is loaded into every Claude Code session, so it must be concise — only include what Claude would get wrong without it.
...
> Quick context:
> - **${promptFile}** files give Claude persistent instructions for a project, your personal workflow, or your organization. Claude reads them at the start of every session.
...
- Q1: "Which ${promptFile} files should /init set up?"
  Options: "Project ${promptFile}" | "Personal CLAUDE.local.md" | "Both project + personal" | "Let Claude decide"
  Description for project: "Team-shared instructions checked into source control — architecture, coding standards, common workflows."
  Description for personal: "Your private preferences for this project (gitignored, not shared) — your role, sandbox URLs, preferred test data, workflow quirks."
  Description for Let Claude decide: "Fastest path — project ${promptFile} plus whatever skills or hooks fit this repo. No follow-on questions; you'll approve everything before it's written."
  If the user picks "Let Claude decide", skip Q2 — treat it as project ${promptFile} with no skills/hooks constraint.
...
If the user chose project ${promptFile}, both, or "Let Claude decide": ask about codebase practices — non-obvious commands, gotchas, branch/PR conventions, required env setup, testing quirks. Skip things already in README or obvious from manifest files. Do not mark any options as "recommended" — this is about how their team works, not best practices.
...
Write a minimal ${promptFile} at the project root. Every line must pass this test: "Would removing this cause Claude to make mistakes?" If no, cut it.
...
**Consume \`note\` entries from the Phase 3 preference queue whose target is ${promptFile}** (team-level notes) — add each as a concise line in the most relevant section. These are the behaviors the user wants Claude to follow but didn't need guaranteed (e.g., "propose a plan before implementing", "explain the tradeoffs when refactoring"). Leave personal-targeted notes for Phase 5.
...
For projects with distinct subdirectories (monorepos, multi-module projects, etc.): mention that subdirectory ${promptFile} files can be added for module-specific instructions (they're loaded automatically when Claude works in those directories). Offer to create them if the user wants.
...
Tell the user you're going to suggest a few additional optimizations now that ${promptFile} and skills (if chosen) are in place.
...
- **Proposal-sourced hooks** (if the approved proposal includes any): Consume \`hook\` entries from the Phase 3 preference queue. If Phase 2 found a formatter and the queue has no formatting hook, offer format-on-edit as a fallback.
  ...
  1. Target file: default based on the Phase 1 ${promptFile} choice — project → \`.claude/settings.json\` (team-shared, committed); personal → \`.claude/settings.local.json\`. Only ask if the user chose "both" in Phase 1 or the preference is ambiguous. Ask once for all hooks, not per-hook.
...
skillRegistry.register(initSkill);
