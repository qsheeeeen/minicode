...
export async function loadGlobalPrompt(): Promise<string> {
  const globalPromptPath = path.join(os.homedir(), '.minicode', DEFAULT_PROMPT_FILE);
  return readPromptFile(globalPromptPath);
}

export async function loadProjectPrompt(cwd: string, promptFile: string = DEFAULT_PROMPT_FILE): Promise<string> {
