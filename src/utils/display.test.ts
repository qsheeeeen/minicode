...
import { RecordEvents, RecordPrompter, CallbackEvents, CallbackPrompter, ConsoleEvents, ConsolePrompter } from './display.js';
...
describe('RecordPrompter', () => {
  it('records prompt and returns empty string', async () => {
    ...
    const result = await prompter.prompt({ message: 'Test?', options: [{ label: 'Yes', value: 'yes' }] });
    expect(result).toBe('');
