...
export const agentTool: ToolDef = {
  ...
  input_schema: {
    ...
    properties: {
      ...
      tier: {
        type: 'string' as const,
        description: 'Optional: run the sub-agent with the model mapped to this tier ("1", "2", or "3"). Overrides the parent model.'
      }
    },
    required: ['task']
  },
  execute: async (args: Record<string, unknown>, context?: ToolExecutionContext): Promise<ToolResult> => {
    ...
    if (tier) {
      ...
      if (modelSpec) {
        const parsed = parseModelSpecifier(modelSpec, appConfig.providers ?? {});
        if (parsed) {
