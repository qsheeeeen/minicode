...
describe('editTool', () => {
  ...
  describe('execute', () => {
    ...
    it('replaces first occurrence only', async () => {
      const fs = await import('fs/promises');
      (fs.default.readFile as ReturnType<typeof vi.fn>).mockResolvedValue('foo bar foo');
      await editTool.execute({ path: 'test.txt', oldText: 'foo', newText: 'baz' });
      const written = (fs.default.writeFile as ReturnType<typeof vi.fn>).mock.calls[0][1];
    ...
    it('throws when oldText not found', async () => {
      const fs = await import('fs/promises');
      (fs.default.readFile as ReturnType<typeof vi.fn>).mockResolvedValue('hello world');
      const result = await editTool.execute({ path: 'test.txt', oldText: 'notfound', newText: 'replacement' });
    ...
    it('returns error when file not found', async () => {
      ...
      const result = await editTool.execute({ path: 'nonexistent.txt', oldText: 'old', newText: 'new' });
