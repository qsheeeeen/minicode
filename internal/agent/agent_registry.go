...
func (r *AgentRegistry) Register(id string, agent any, task string, parentID string) {
	ag := agent.(*Agent)
	s := &AgentSession{
		ID:       id,
		Type:     AgentSub,
...
// ActiveAgent returns the active agent as any (for interface compatibility).
func (r *AgentRegistry) ActiveAgent() any {
	return r.Active()
}
...
// OnUpdate sets a function called whenever sessions change.
...
func (r *AgentRegistry) UpdateStatus(id string, status string, summary string) {
	r.mu.Lock()
	if s, ok := r.sessions[id]; ok {
